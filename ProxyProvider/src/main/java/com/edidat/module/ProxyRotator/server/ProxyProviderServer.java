...
/**
 * @author rohit.ihare
 * {@link ProxyProviderServer} is the server application which provides the free proxies available across Internet with the help of free-APIs and web scrapers.
 * It servers the client request over tcp connection and communicates using json. 
 */
public class ProxyProviderServer {
	...
	/**
	 * Init the proxy queue and start filling it with extracted proxies
	 * Also starts the tcp server to serve client requests.
	 * @throws Exception
	 */
	...
	/**
	 * It starts the tcp server and accepts proxy requests from client in json format.
	...
	public static void clientRequestServer() throws ProxyRotatorException {
		...
		while (!shutdown) {
			try {
				/* TCP Server accepts clients and create separate thread for handling clients requests */
